...
pub struct ServerConfig {
    ...
    pub bind: String,
    /// Path to the X25519 private key file. In auto-bootstrap mode the
    /// server writes the key to `./server.key` and this field can be left
    /// blank; explicit `phantom server -c <file>` configs can set it to
    /// point at any absolute path.
    #[serde(default)]
    pub private_key: String,
    /// Optional path to a client-whitelist file. When `allowed_clients` is
    /// non-empty it takes precedence; when `clients` is `None`/empty the
    /// server runs in OPEN mode.
    #[serde(default)]
    pub clients: Option<String>,
    /// Inline client whitelist. When non-empty, overrides `clients`.
    /// Each entry is a base64 X25519 public key with an optional `name` label.
    #[serde(default)]
...
impl ServerConfig {
    ...
    /// Load the client public key whitelist. Precedence:
    /// 1. Inline `allowed_clients` (if non-empty)
    /// 2. The `clients` file path (if set and the file exists)
    /// 3. OPEN mode (empty vec)
    pub fn load_allowed_clients(&self) -> Result<Vec<[u8; 32]>> {
        if !self.allowed_clients.is_empty() {
            return self
                .allowed_clients
                .iter()
                .map(|c| decode_key(&c.public_key))
                .collect();
        }
        match &self.clients {
            Some(p) if !p.is_empty() => Self::load_allowed_clients_from(p),
            _ => Ok(Vec::new()),
        }
    }

    /// Load the client public key whitelist from an arbitrary path.
    /// Unlike [`Self::load_allowed_clients`], this does not consult `self.clients`.
    /// Used by the bootstrap path where the file lives outside a TOML config.
    pub fn load_allowed_clients_from(path: &str) -> Result<Vec<[u8; 32]>> {
        let content = fs::read_to_string(path)
