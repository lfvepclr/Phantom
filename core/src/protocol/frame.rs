...
impl Frame {
    ...
    pub fn encode(&self) -> Bytes {
        ...
        assert!(payload_len <= crate::constants::MAX_FRAME_PAYLOAD);

        let mut buf = BytesMut::with_capacity(crate::constants::FRAME_HEADER_SIZE + payload_len);
    ...
    pub fn decode(data: Bytes) -> Result<Self> {
        if data.len() < crate::constants::FRAME_HEADER_SIZE {
...
mod boundary_tests {
    ...
    fn udp_syn_frame_roundtrip() {
        let frame = Frame {
            ...
            payload: Bytes::from(&b"udp-syn-data"[..]),
        };
        let encoded = frame.encode();
        let decoded = Frame::decode(encoded).unwrap();
        assert_eq!(decoded.stream_id, 7);
        assert!(decoded.flags.contains(FrameFlags::SYN));
        assert!(decoded.flags.contains(FrameFlags::UDP));
        assert!(decoded.flags.contains(FrameFlags::DATA));
        assert_eq!(&decoded.payload[..], b"udp-syn-data");
    }

    #[test]
    fn udp_data_frame_roundtrip() {
        let frame = Frame {
            version: PROTOCOL_VERSION,
            stream_id: 3,
            flags: FrameFlags::UDP | FrameFlags::DATA,
            payload: Bytes::from(&b"datagram"[..]),
        };
        let encoded = frame.encode();
        let decoded = Frame::decode(encoded).unwrap();
        assert!(decoded.flags.contains(FrameFlags::UDP));
    ...
    fn max_payload_frame() {
        let frame = Frame::data(1, vec![0xABu8; MAX_FRAME_PAYLOAD]);
        let encoded = frame.encode();
        let decoded = Frame::decode(encoded).unwrap();
