...
1. **第二台 Mac**（`user@<内网IP>`，密码不写入任何文档）：首连部署公钥免密 → 探测机型/芯片/网口速率（1GbE→118 / 2.5GbE→295 MB/s 封顶）→ 源码构建服务端 → LAN 线速对测（本机 phantom client ↔ 对端 phantom server，打满网口）。
...
1. **G1 路由器 client 吞吐实测**：本机 `curl -x socks5h://router:***@<路由器IP>:1080` 拉 <内网主机>:9090/testfile.bin ×3，同时采样路由器 CPU%（top）+ VmRSS（/proc/PID/status）。基线对比：Mac client 双跳 20.3 MB/s、路由器 server 单跳 56.7 MB/s。
