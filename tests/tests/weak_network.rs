...
async fn setup_tunnel_with_proxy(
    fixture: &TestFixture, proxy_addr: std::net::SocketAddr,
) -> anyhow::Result<(
    ...
    connect_tunnel(proxy_addr, &fixture.server_key.public, &fixture.client_key.secret, &target, fixture.cipher_preference).await
}

#[tokio::test]
async fn weak_net_latency_50ms() {
    let fixture = TestFixture::new(CipherPreference::Aes256Gcm).await;
    let proxy = ThrottledProxy::start(fixture.server_addr, NetworkCondition::high_latency(50)).await;
    let (mut reader, mut writer, stream_id) = setup_tunnel_with_proxy(&fixture, proxy.listen_addr).await.unwrap();
...
async fn weak_net_latency_200ms() {
    let fixture = TestFixture::new(CipherPreference::Aes256Gcm).await;
    let proxy = ThrottledProxy::start(fixture.server_addr, NetworkCondition::high_latency(200)).await;
    let (mut reader, mut writer, stream_id) = setup_tunnel_with_proxy(&fixture, proxy.listen_addr).await.unwrap();
    let data = generate_random_data(WEAK_NET_TEST_SIZE);
    let echoed = echo_data(&mut reader, &mut writer, stream_id, &data).await;
...
async fn weak_net_latency_500ms() {
    let fixture = TestFixture::new(CipherPreference::Aes256Gcm).await;
    let proxy = ThrottledProxy::start(fixture.server_addr, NetworkCondition::high_latency(500)).await;
    let (mut reader, mut writer, stream_id) = setup_tunnel_with_proxy(&fixture, proxy.listen_addr).await.unwrap();
...
async fn weak_net_bandwidth_10mbps() {
    let fixture = TestFixture::new(CipherPreference::Aes256Gcm).await;
    let proxy = ThrottledProxy::start(fixture.server_addr, NetworkCondition::low_bandwidth(10_000_000 / 8)).await;
    let (mut reader, mut writer, stream_id) = setup_tunnel_with_proxy(&fixture, proxy.listen_addr).await.unwrap();
    let data = generate_random_data(WEAK_NET_TEST_SIZE);
    let echoed = echo_data(&mut reader, &mut writer, stream_id, &data).await;
...
async fn weak_net_bandwidth_1mbps() {
    let fixture = TestFixture::new(CipherPreference::Aes256Gcm).await;
    let proxy = ThrottledProxy::start(fixture.server_addr, NetworkCondition::low_bandwidth(1_000_000 / 8)).await;
    let (mut reader, mut writer, stream_id) = setup_tunnel_with_proxy(&fixture, proxy.listen_addr).await.unwrap();
    let data = generate_random_data(256 * 1024);
    let echoed = echo_data(&mut reader, &mut writer, stream_id, &data).await;
...
async fn weak_net_loss_1pct() {
    let fixture = TestFixture::new(CipherPreference::Aes256Gcm).await;
    let proxy = ThrottledProxy::start(fixture.server_addr, NetworkCondition::packet_loss(1.0)).await;
    let (mut reader, mut writer, stream_id) = setup_tunnel_with_proxy(&fixture, proxy.listen_addr).await.unwrap();
    let data = generate_random_data(WEAK_NET_TEST_SIZE);
    let echoed = echo_data(&mut reader, &mut writer, stream_id, &data).await;
...
async fn weak_net_loss_5pct() {
    let fixture = TestFixture::new(CipherPreference::Aes256Gcm).await;
    let proxy = ThrottledProxy::start(fixture.server_addr, NetworkCondition::packet_loss(5.0)).await;
    let (mut reader, mut writer, stream_id) = setup_tunnel_with_proxy(&fixture, proxy.listen_addr).await.unwrap();
    let data = generate_random_data(256 * 1024);
    let echoed = echo_data(&mut reader, &mut writer, stream_id, &data).await;
...
async fn weak_net_moderate() {
    let fixture = TestFixture::new(CipherPreference::Aes256Gcm).await;
    let proxy = ThrottledProxy::start(fixture.server_addr, NetworkCondition::combined(100, 10_000_000 / 8, 1.0)).await;
    let (mut reader, mut writer, stream_id) = setup_tunnel_with_proxy(&fixture, proxy.listen_addr).await.unwrap();
...
async fn weak_net_jitter() {
    let fixture = TestFixture::new(CipherPreference::Aes256Gcm).await;
    let proxy = ThrottledProxy::start(fixture.server_addr, NetworkCondition::high_latency(100).with_jitter(50)).await;
    let (mut reader, mut writer, stream_id) = setup_tunnel_with_proxy(&fixture, proxy.listen_addr).await.unwrap();
    let data = generate_random_data(WEAK_NET_TEST_SIZE);
    let echoed = echo_data(&mut reader, &mut writer, stream_id, &data).await;
...
async fn weak_net_200ms_aes256gcm_vs_ascon128() {
    ...
    eprintln!("\n===== Weak Network Cipher Comparison (200ms latency, {} KB) =====", test_size / 1024);
    for (cipher, name) in [(CipherPreference::Aes256Gcm, "AES-256-GCM"), (CipherPreference::Ascon128, "ASCON-128")] {
        ...
        let (mut reader, mut writer, stream_id) = connect_tunnel(
            proxy.listen_addr, &fixture.server_key.public, &fixture.client_key.secret, &target, fixture.cipher_preference,
        ).await.unwrap();
