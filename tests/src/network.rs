...
impl NetworkCondition {
    ...
    pub fn high_latency(ms: u64) -> Self {
        Self {
            latency_ms: ms,
            jitter_ms: 0,
            bandwidth_bps: 0,
    ...
    pub fn low_bandwidth(bps: u64) -> Self {
        Self {
            latency_ms: 0,
            jitter_ms: 0,
            bandwidth_bps: bps,
    ...
    pub fn packet_loss(pct: f64) -> Self {
        Self {
            latency_ms: 0,
            jitter_ms: 0,
            bandwidth_bps: 0,
...
impl ThrottledProxy {
    pub async fn start(target_addr: SocketAddr, condition: NetworkCondition) -> Self {
        let listener = TcpListener::bind("127.0.0.1:0")
            .await
            .expect("Failed to bind throttled proxy");
        ...
        loop {
            ...
            if cond_c2s.bandwidth_bps > 0 {
                ...
                for start in (0..n).step_by(chunk) {
                    ...
                    tokio::io::AsyncWriteExt::write_all(&mut server_write, &buf[start..end])
                        .await?;
                    let d = Duration::from_secs_f64(
                        (end - start) as f64 / cond_c2s.bandwidth_bps as f64,
                    );
                    if !d.is_zero() {
                        sleep(d).await;
                    }
    ...
    let s2c = async {
        ...
        loop {
            ...
            if cond_s2c.bandwidth_bps > 0 {
                ...
                for start in (0..n).step_by(chunk) {
                    let end = cmp::min(start + chunk, n);
                    tokio::io::AsyncWriteExt::write_all(&mut client_write, &buf[start..end])
                        .await?;
                    let d = Duration::from_secs_f64(
                        (end - start) as f64 / cond_s2c.bandwidth_bps as f64,
                    );
                    if !d.is_zero() {
                        sleep(d).await;
                    }
