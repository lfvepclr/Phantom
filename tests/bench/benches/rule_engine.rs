...
const PORTS: &[u16] = &[
    22, 53, 80, 123, 135, 137, 138, 139, 161, 389, 443, 445, 465, 500, 514,
    587, 636, 873, 902, 989, 990, 993, 995, 1080, 1194, 1433, 1521, 1701,
    1723, 1812, 1900, 2049, 2082, 2083, 2086, 2087, 2095, 2096, 2181, 2375,
    2376, 3000, 3306, 3389, 4500, 4848, 5000, 5060, 5432, 5601, 5672, 5900,
    5984, 6379, 6443, 7001, 7474, 8000, 8080, 8081, 8443, 8500, 8888, 9000,
    9092, 9200, 9418, 11211, 15672, 26379, 27017, 50070,
];
...
fn query_mixed_no_suffix_hit(bencher: Bencher, n: usize) {
    ...
    bencher.bench_local(|| {
        let mut acc = 0u8;
        for (d, ip) in probe_domains.iter().cycle().take(20).zip(QUERY_IPS_V4.iter().cycle()) {
            acc = acc.wrapping_add(engine.query(Some(d), Some(*ip), Some(443)) as u8);
        }
        acc
    });
