...
async fn debug_raw_echo_server() {
    ...
    let mut stream = tokio::net::TcpStream::connect(fixture.target_addr).await.unwrap();
    ...
    eprintln!("Echo server returned: {} bytes: {:?}", n, String::from_utf8_lossy(&buf[..n]));
...
async fn debug_echo_via_tunnel_no_fin() {
    ...
    writer.write_frame(&Frame::data(stream_id, Bytes::from(&b"hello"[..]))).await.unwrap();
    ...
    eprintln!("Got frame: flags={:?}, data={:?}", frame.flags, String::from_utf8_lossy(&frame.payload));
    ...
    let fin_result = tokio::time::timeout(
